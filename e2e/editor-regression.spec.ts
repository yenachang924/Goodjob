...
async function navigate(page: Page, name: string) {
  ...
  await page
    ...
    .click();
  if (name === '오늘') {
    const details = page.locator('.planning-details');
...
async function openProject(page: Page, name: string) {
  ...
  await expect(
    page.getByRole('button', { name: '프로젝트 추가' }),
  ).toBeVisible();
...
async function addTask(page: Page, title: string) {
  ...
  await page.getByLabel('예상 시간', { exact: true }).fill('30');
  await page.getByRole('button', { name: '작업 저장' }).click();
  await expect(page.getByLabel('작업 이름')).toBeHidden();
}

test('milestone draft cannot switch targets until saved or cancelled', async ({
  page,
}, info) => {
...
test('task draft cannot switch targets until saved or cancelled', async ({
  page,
}, info) => {
  ...
  await page.getByRole('button', { name: '취소', exact: true }).click();
  await expect(page.locator('.control-task-list')).toContainText(
    '보존할 작업 초안',
  );
});

test('a child inherits an explicitly unassigned parent milestone', async ({
  page,
}, info) => {
  ...
  const milestone = page.getByRole('combobox', {
    name: '마일스톤',
    exact: true,
  });
  ...
  await page
    ...
    .selectOption({ label: '목표 없는 부모' });
  await expect(milestone).toHaveValue('');
  await expect(milestone).toBeDisabled();
  await page.getByRole('button', { name: '작업 저장' }).click();
  await expect(page.getByLabel('작업 이름')).toBeHidden();
  await expect(page.locator('.control-task.child')).toContainText(
    '상속할 하위 작업',
  );
});

test('a historical confirmed plan can be adjusted before decomposing its task', async ({
  page,
}, info) => {
  ...
  await page
    .getByRole('button', { name: '+ 과거 계획의 부모', exact: true })
    .click();
  await page.getByRole('button', { name: '이 계획 확정' }).click();
  await expect(
    page.getByRole('heading', { name: '확정된 계획' }),
  ).toBeVisible();
  ...
  await page
    .getByRole('combobox', { name: '상위 작업', exact: true })
    .selectOption({ label: '과거 계획의 부모' });
  await page.getByRole('button', { name: '작업 저장' }).click();
  await expect(
    page.getByRole('alert').filter({ hasText: '계획' }),
  ).toBeVisible();
  await page.getByRole('button', { name: '취소', exact: true }).click();
  await navigate(page, '오늘');
  await date.fill('2026-01-05');
...
test('pending plan save locks inputs and navigation until its real response', async ({
  page,
}, info) => {
  ...
  const navigationToggle = page.getByRole('button', { name: '프로젝트 목록 펼치기', exact: true });
  ...
  try {
    ...
    await expect(
      page.getByRole('tab', { name: '시간 기록', exact: true }),
    ).toBeDisabled();
  } finally {
    release();
  }
...
test('recommendation confirmation persists and overbudget edits preserve the saved plan', async ({
  page,
}, info) => {
  ...
  for (const [index, title] of titles.entries()) {
    ...
    await page
      .getByLabel('마감일', { exact: true })
      .fill(index === 1 ? '2026-01-02' : '2026-01-01');
    if (index === 2)
      await page
  ...
  await page.getByRole('button', { name: '이 계획 확정' }).click();
  await expect(
    page.getByRole('alert').filter({ hasText: '입력값' }),
  ).toBeVisible();
  await page.reload();
  await navigate(page, '오늘');
  await expect(
    page.getByLabel('등록한 가용시간 · 분', { exact: true }),
  ).toHaveValue('70');
  await expect(page.locator('.plan-row strong')).toHaveText(titles.slice(0, 2));
});

test('pending time record save cannot open a replacement draft', async ({
  page,
}, info) => {
  ...
  await page
    ...
    .selectOption({ label: '시간 기록 잠금 작업' });
  await page
    .getByRole('checkbox', { name: '시작·종료 직접 지정', exact: true })
    .check();
  ...
  try {
    ...
    await expect(
      page.getByRole('button', { name: '수동 기록 추가', exact: true }),
    ).toBeDisabled();
    await expect(
  ...
  await expect(
    page.locator('.record-row').filter({ hasText: '시간 기록 잠금 작업' }),
  ).toContainText('30분');
  await expect(
    page.getByRole('button', { name: '수동 기록 추가', exact: true }),
  ).toBeEnabled();
...
test('future plan dates render zero recorded time without crashing', async ({
  page,
}, info) => {
  ...
  await expect(
    page.getByRole('heading', { name: '2099-01-01 계획' }),
  ).toBeVisible();
  await expect(page.locator('.today-summary article').last()).toContainText(
    '00:00:00',
  );
});
