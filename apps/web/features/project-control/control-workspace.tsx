...
import { useControlWorkspace } from './use-control-workspace';
import './control.css';
...
export default function ControlWorkspace() {
    ...
    return (
      <main className="control-shell">
        <div className="loading-card">
          ...
          <h1>프로젝트를 불러오는 중</h1>
          <p>클라우드에 저장된 워크스페이스를 확인하고 있어요.</p>
        </div>
      </main>
    );
  if (!workspace.data)
  ...
  async function importFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return;
    try {
      if (file.size > MAX_BACKUP_BYTES) throw new Error('백업 파일 크기가 저장 한도를 초과했습니다.');
  ...
  return (
    <main className="control-shell">
      <fieldset disabled={workspace.busy} className="control-save-boundary" aria-label="워크스페이스 편집">
      <header className="control-header">
        ...
        <div className="header-tools">
          <span className="cloud-state">
            {workspace.busy ? '저장 중…' : workspace.notice || '클라우드 연결'}
          </span>
          <Button
            size="sm"
            variant="ghost"
