...
> 아래는 기존 관제판의 Vercel/Supabase 이전 결정이다. 이후 **백엔드 학습**이라는 별도 목표가 추가되어 `apps/api`에 Kotlin/Spring Boot 서버와 `/learn/tasks`를 분리했다. 이 변경의 선택 이유와 실행 경계는 [백엔드 학습실](backend-learning.md)을 참고한다. 기존 관제판의 저장 경로와 배포는 그대로다.
